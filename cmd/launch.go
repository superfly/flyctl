...
func runLaunch(cmdCtx *cmdctx.CmdContext) error {
	...
	if fromUrl := cmdCtx.Config.GetString("from"); fromUrl != "" {
		...
		// If the remote fly.toml wants a disk, reuse srcInfo from the scanner to set one up.
		// This approach is silly, but works.
		// Maybe this points to us needing a better way to unmarshal TOML to structured types from all fly.toml sections.
		...
		if strings.Contains(fromUrl, "redis") {
			srcInfo.Secrets = map[string]string{
				"REDIS_PASSWORD": "The password used to access Redis. Use something long and random. Save the password somewhere.",
			}
		}
	...
	// If we haven't imported the app config, use the default generated by the API
	if !importedConfig {
	...
	// Finally, write the config unless we've specified not to, for example when launching from a remote fly.toml
	if writeConfig {
