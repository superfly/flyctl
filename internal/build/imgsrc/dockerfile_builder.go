...
func (*dockerfileBuilder) Run(ctx context.Context, dockerFactory *dockerClientFactory, streams *iostreams.IOStreams, opts ImageOptions, build *build) (*DeploymentImage, string, error) {
	...
	buildkitEnabled, err := buildkitEnabled(client)
	...
	defer func() {
		...
		if dockerFactory.IsLocal() {
			clearDeploymentTags(ctx, client, opts.Tag)
		}
	}()
	...
	if buildkitEnabled {
		imageID, err = runBuildKitBuild(ctx, client, opts, dockerfile, buildArgs)
		if err != nil {
			if dockerFactory.IsRemote() {
				metrics.SendNoData(ctx, "remote_builder_failure")
			}
		...
		imageID, err = runClassicBuild(ctx, streams, client, buildContext, opts, relDockerfile, buildArgs)
		if err != nil {
			if dockerFactory.IsRemote() {
				metrics.SendNoData(ctx, "remote_builder_failure")
			}
