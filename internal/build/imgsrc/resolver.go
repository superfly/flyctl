...
func (r *Resolver) StartHeartbeat(ctx context.Context) (*StopSignal, error) {
	...
	client, err := r.dockerFactory.buildFn(ctx, nil)
	if err != nil {
		terminal.Warnf(errMsg, err)
		return nil, nil
	}
	heartbeatUrl, err := getHeartbeatUrl(client)
	if err != nil {
		terminal.Warnf(errMsg, err)
	...
	err = heartbeat(ctx, client, heartbeatReq)
	if err != nil {
		var h *httpError
		if errors.As(err, &h) {
	...
	resp, err := client.HTTPClient().Do(heartbeatReq)
	if err != nil {
