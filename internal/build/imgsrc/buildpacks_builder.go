...
func (*buildpacksBuilder) Run(ctx context.Context, dockerFactory *dockerClientFactory, streams *iostreams.IOStreams, opts ImageOptions, build *build) (*DeploymentImage, string, error) {
	...
	span.SetAttributes(attribute.String("deployment_image.id", img.ID))
	span.SetAttributes(attribute.String("deployment_image.tag", opts.Tag))
	span.SetAttributes(attribute.Int64("deployment_image.size", img.Size))
