...
func TestToMachineConfig(t *testing.T) {
	...
	assert.NotEmpty(t, got.Metadata[api.MachineConfigMetadataKeyFlyctlVersion])
	delete(got.Metadata, api.MachineConfigMetadataKeyFlyctlVersion)
...
func TestToReleaseMachineConfig(t *testing.T) {
	...
	// this is an autogenerated value, so it's going to differ across flyctl versions and i don't know of a simple way to mock it.
	// so i just assert that it's not empty and delete it before we do the diff comparison.
	assert.NotEmpty(t, got.Metadata[api.MachineConfigMetadataKeyFlyctlVersion])
	delete(got.Metadata, api.MachineConfigMetadataKeyFlyctlVersion)
