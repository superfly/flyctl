		...
		defaults.availableVolumeCounts = lo.MapValues(
			lo.GroupBy(
				lo.Filter(volumes, func(v fly.Volume, _ int) bool {
					return v.State == "created" && !v.IsAttached() && v.HostStatus == "ok"
				}),
...
func (d *defaultValues) takeAvailableVolumeCount(mConfig *fly.MachineConfig, region string, delta int) int {
