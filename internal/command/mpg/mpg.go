...
func ClusterFromArgOrSelect(ctx context.Context, clusterID, orgSlug string) (*mpg.Cluster, string, error) {
	...
	// If the cluster ID is provided, try to retrieve it from the v1 client first
	// then the v2 client. If both fail, then we just fall through to the default
	// logic
	if clusterID != "" {
		...
		if c, err := mpgv2Client.GetManagedClusterById(ctx, clusterID); err == nil {
			cluster := &mpg.Cluster{
				Id:            c.Data.Id,
				Name:          c.Data.Name,
				Region:        c.Data.Region,
