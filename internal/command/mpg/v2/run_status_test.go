...
func TestRunStatusHuman(t *testing.T) {
	...
	wantPublicColumns := map[string]string{
		...
		"Used Storage (GiB)": "1.2", "Allocated Storage (GiB)": "20.0",
	...
	tests := []struct {
		...
		{"classified 404 falls back to legacy with full mapping",
			...
			sampleLegacyCluster(), nil, map[string]string{"ID": "mpg-123", "Name": "test-cluster", "Organization": "test-org", "Region": "ord", "Status": "ready", "Used Storage (GiB)": "1.2", "Allocated Storage (GiB)": "20.0", "Replicas": "1", "Direct IP": "10.0.0.1"}, wantPublicExcludes, "", true, "", 1},
