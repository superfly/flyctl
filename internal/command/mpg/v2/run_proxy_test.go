...
func TestProxyParamsRequireDirectIP(t *testing.T) {
	cluster, params, err := proxyParams(&mpgv2.GetClusterResponse{}, nil, "15432", "test-org", "127.0.0.1", nil)
...
// TestProxyParamsPublicShape verifies the public-API response is adapted into
// the legacy shape with a bare-host RemoteHost (port carried by Ports[1]).
...
func TestResolveDefaultConnectCredentials(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			response := &mpgv2.GetClusterResponse{
				Data: mpgv2.ManagedCluster{Name: name, Status: tt.clusterSt},
				Credentials: mpgv2.GetClusterCredentialsResponse{
...
func TestResolveExplicitUserConnectCredentialsErrors(t *testing.T) {
	...
	t.Run("empty password", func(t *testing.T) {
		...
		response := &mpgv2.GetClusterResponse{
			Data: mpgv2.ManagedCluster{Name: name, Status: "ready"},
		}
		credentials, err := resolveConnectCredentials(ctx, response, true, "app-user")
...
func TestResolveConnectCredentialsPublic(t *testing.T) {
	response := &mpgv2.GetClusterResponse{
		Data: mpgv2.ManagedCluster{Id: "cluster-id", Name: "test-cluster", Status: "ready"},
	}
