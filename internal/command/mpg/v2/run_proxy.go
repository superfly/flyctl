...
func GetMpgConnectParams(
	...
	clusterID string,
	resolvedOrgSlug string,
) (*mpgv2.ManagedCluster, bool, *proxy.ConnectParams, *mpgv2.GetClusterCredentialsResponse, error) {
...
// back to the legacy MPGv2 client only when the public call returns a
...
// mpgv2.ManagedCluster type, since that type is part of the private/legacy
...
// separately (not as a field on the legacy mpgv2.ManagedCluster type) so
