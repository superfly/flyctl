...
// Copied from v1 RunCreate and changed client to mpgv2.
// Signature differs from v1 because v2 create is not yet wired up in create.go.
// When wired up, the caller should pass the appropriate params.
func RunCreate(ctx context.Context, org *fly.Organization, appName string) error {
	io := iostreams.FromContext(ctx)
	mpgClient := mpgv2.ClientFromContext(ctx)

	// NOTE: v2 create is not yet fully wired up in the command layer (create.go).
	// This implementation mirrors v1's RunCreate but uses the v2 client.
	// The caller needs to be updated to pass CreateClusterParams and CreatePlanDisplay
	// once v2 create is integrated into create.go.
	_ = io
	_ = mpgClient
	return fmt.Errorf("v2 cluster creation is not yet wired up - use v1 create for now")
}

// RunCreateWithParams is the full v2 create implementation, copied from v1 and changed client to mpgv2.
// Use this when wiring up v2 create in the command layer.
func RunCreateWithParams(ctx context.Context, params *CreateClusterParams, planDisplay *CreatePlanDisplay) error {
	io := iostreams.FromContext(ctx)
	mpgClient := mpgv2.ClientFromContext(ctx)
