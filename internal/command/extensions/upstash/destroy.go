...
func runDestroy(ctx context.Context) (err error) {
	...
	extension, _, err := extensions_core.Discover(ctx, gql.AddOnTypeUpstashRedis)
	...
	if !flag.GetYes(ctx) {
		const msg = "Destroying a database is not reversible."
		fmt.Fprintln(io.ErrOut, colorize.Red(msg))

		switch confirmed, err := prompt.Confirmf(ctx, "Destroy database %s?", extension.Name); {
		case err == nil:
			if !confirmed {
				return nil
			}
	...
	fmt.Fprintf(out, "Your database %s was destroyed\n", extension.Name)
