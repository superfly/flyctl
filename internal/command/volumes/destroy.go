...
func confirmVolumeDelete(ctx context.Context, volID string) (bool, error) {
	...
	if matches <= 2 {
		msg = fmt.Sprintf("Warning! Every volume is pinned to a specific physical host. You should create two or more volumes per application to avoid downtime and permanent data loss. Deleting this volume will leave you with %d volume(s) for this application, and it is not reversible. Learn more at https://fly.io/docs/reference/volumes/", matches-1)
	}
