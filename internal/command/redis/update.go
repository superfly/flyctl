...
func runUpdate(ctx context.Context) (err error) {
	...
	prodPack, err := resolveProdPack(
		...
		func(currentlyOn bool) (bool, error) {
...
// only decides which way the prompt is phrased. Declining the prompt means
// "leave it alone" rather than "set the opposite", so a stale stored copy can
// never flip a setting the user did not ask to change. A non-interactive
// session makes no decision at all unless a flag was passed.
func resolveProdPack(enableFlag, disableFlag bool, stored any, selectedPlanIsLegacy bool, confirm func(currentlyOn bool) (bool, error)) (*bool, error) {
