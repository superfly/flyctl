...
type ResolverCreateBuildCreateBuildCreateBuildPayload struct {
	SourceBuild ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild `json:"sourceBuild"`
}

// GetSourceBuild returns ResolverCreateBuildCreateBuildCreateBuildPayload.SourceBuild, and is useful for accessing the field via an interface.
func (v *ResolverCreateBuildCreateBuildCreateBuildPayload) GetSourceBuild() ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild {
	return v.SourceBuild
}

// ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild includes the requested fields of the GraphQL type SourceBuild.
type ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild struct {
	Id string `json:"id"`
	// Status of the build
	Status string `json:"status"`
}

// GetId returns ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild.Id, and is useful for accessing the field via an interface.
func (v *ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild) GetId() string { return v.Id }

// GetStatus returns ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild.Status, and is useful for accessing the field via an interface.
func (v *ResolverCreateBuildCreateBuildCreateBuildPayloadSourceBuild) GetStatus() string {
	return v.Status
}
...
type ResolverFinishBuildFinishBuildFinishBuildPayload struct {
	SourceBuild ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild `json:"sourceBuild"`
}

// GetSourceBuild returns ResolverFinishBuildFinishBuildFinishBuildPayload.SourceBuild, and is useful for accessing the field via an interface.
func (v *ResolverFinishBuildFinishBuildFinishBuildPayload) GetSourceBuild() ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild {
	return v.SourceBuild
}

// ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild includes the requested fields of the GraphQL type SourceBuild.
type ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild struct {
	Id string `json:"id"`
	// Status of the build
	Status  string                                                             `json:"status"`
	Timings ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuildTimings `json:"timings"`
}

// GetId returns ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild.Id, and is useful for accessing the field via an interface.
func (v *ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild) GetId() string { return v.Id }

// GetStatus returns ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild.Status, and is useful for accessing the field via an interface.
func (v *ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild) GetStatus() string {
	return v.Status
}

// GetTimings returns ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild.Timings, and is useful for accessing the field via an interface.
func (v *ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuild) GetTimings() ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuildTimings {
	return v.Timings
}

// ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuildTimings includes the requested fields of the GraphQL type BuildTimings.
type ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuildTimings struct {
	// Time from creating build to finishing build, all inclusive, measured server side
	WallclockTimeMs int64 `json:"wallclockTimeMs"`
}

// GetWallclockTimeMs returns ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuildTimings.WallclockTimeMs, and is useful for accessing the field via an interface.
func (v *ResolverFinishBuildFinishBuildFinishBuildPayloadSourceBuildTimings) GetWallclockTimeMs() int64 {
