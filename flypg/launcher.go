...
func (l *Launcher) LaunchMachinesPostgres(ctx context.Context, config *CreateClusterInput, detach bool) error {
	...
	if config.BarmanRemoteRestoreConfig != "" && config.InitialClusterSize != 1 {
		fmt.Fprintf(io.Out, colorize.Yellow("Warning: Backups can only be restored to single instances. Forcing cluster size to 1 for restore\n"))
		config.InitialClusterSize = 1
	}
	...
	for i := 0; i < config.InitialClusterSize; i++ {
		...
		if backupSourceAppName != "" {
			flapsClient, err := flapsutil.NewClientWithOptions(ctx, flaps.NewClientOpts{
				AppName: backupSourceAppName,
			})
			if err != nil {
				return err
			}
			...
			machine := machines[0]
			machineConf = helpers.Clone(machine.Config)
			if len(machineConf.Mounts) > 0 {
				mount := machineConf.Mounts[0]
				config.VolumeSize = &mount.SizeGb
			}
			machineConf.Mounts = []fly.MachineMount{}
		} else {
