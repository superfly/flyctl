...
func createTigrisBucket(ctx context.Context, config CreateClusterInput) (map[string]interface{}, error) {
	var (
...
func (l *Launcher) LaunchMachinesPostgres(ctx context.Context, config *CreateClusterInput, detach bool) error {
	...
	environment, err := createTigrisBucket(ctx, *config)
	if err != nil {
		return err
	}

	accessKeyId, ok := environment["AWS_ACCESS_KEY_ID"].(string)
	if !ok || accessKeyId == "" {
		return fmt.Errorf("AWS_ACCESS_KEY_ID is unset")
	}

	accessSecret, ok := environment["AWS_SECRET_ACCESS_KEY"].(string)
	if !ok || accessSecret == "" {
		return fmt.Errorf("AWS_SECRET_ACCESS_KEY is unset")
	}

	endpoint, ok := environment["AWS_ENDPOINT_URL_S3"].(string)
	if !ok || endpoint == "" {
		return fmt.Errorf("AWS_ENDPOINT_URL_S3 is unset")
	}

	bucketName, ok := environment["BUCKET_NAME"].(string)
	if !ok || bucketName == "" {
